...
def crop_img(img, ratio):
    ...
    sr = iw/ih
    if sr > ratio:
        nw = int(ih*ratio); off=(iw-nw)//2; img=img.crop((off,0,off+nw,ih))
    else:
        nh = int(iw/ratio); off=(ih-nh)//2; img=img.crop((0,off,iw,off+nh))
    buf = io.BytesIO()
...
# ── Header ────────────────────────────────────────────────────────────────
def draw_header(c, job, page, total):
    logo = load_logo()
    c.setFillColor(NAVY); c.rect(0,H-1.08*inch,W,1.08*inch,fill=1,stroke=0)
    c.setFillColor(SKY); c.rect(0,H-0.05*inch,W,0.05*inch,fill=1,stroke=0)
    tx = M
    if logo:
        buf = io.BytesIO(logo)
    ...
    c.setFillColor(TEAL); c.rect(0,H-1.28*inch,W,0.20*inch,fill=1,stroke=0)
    labels = {1:'HVAC POST-SERVICE REPORT - JOB SUMMARY & SERVICES',
    ...
    tech = job.get('tech','-')
    c.drawRightString(W-M,H-1.21*inch,f'{date}   *   Tech: {tech}   *   Page {page} of {total}')
...
def sec_hdr(c, y, title):
    c.setFillColor(TEAL); c.rect(M,y-0.03*inch,0.032*inch,0.20*inch,fill=1,stroke=0)
...
def info_cards(c, y, job):
    cw=(CW-3*0.09*inch)/4; ch=0.54*inch
    cards=[
    ...
    for i,(lbl,val,sub) in enumerate(cards):
        ...
        if c.stringWidth(val,'SansB',8) > max_w:
            words=val.split(' '); line1=''; line2=''
            for word in words:
                test=(line1+' '+word).strip()
                if c.stringWidth(test,'SansB',8)<=max_w: line1=test
                else: line2=(line2+' '+word).strip()
...
def services_table(c, y, services):
    ...
    c.setFillColor(NAVY); c.rect(M,y-hh,CW,hh,fill=1,stroke=0)
    c.setFont('SansB',7.5); c.setFillColor(WHITE)
    for i,h in enumerate(['COMPONENT / AREA','PRIOR CONDITION','SERVICE PERFORMED','STATUS']):
        c.drawString(cols[i]+0.07*inch,y-hh+0.09*inch,h)
    y-=hh
    for ri,row in enumerate(services):
        comp=row.get('comp','-'); prior=row.get('prior','-')
        svc=row.get('svc','-'); status=row.get('status','Cleaned')
        fg,bg=STATUS_COLORS.get(status,(MID,IVORY))
        c.setFillColor(IVORY if ri%2==0 else WHITE)
        c.rect(M,y-rh,CW,rh,fill=1,stroke=0)
        c.setStrokeColor(LIGHT); c.setLineWidth(0.3); c.line(M,y-rh,M+CW,y-rh)
        c.setFont('SansB',8); c.setFillColor(NAVY)
        ...
        y-=rh
    c.setStrokeColor(TEAL); c.setLineWidth(0.8); c.line(M,y,M+CW,y)
    return y-0.13*inch

# ── Photos ────────────────────────────────────────────────────────────────
def photo_pair(c, y, title, b_img, a_img, b_cap, a_cap, ph):
    pw=(CW-0.15*inch)/2; ratio=pw/ph
    cap_h=0.20*inch; lbl_h=0.13*inch
    c.setFont('SansB',8); c.setFillColor(NAVY); c.drawString(M,y,title)
    c.setStrokeColor(LIGHT); c.setLineWidth(0.3)
    tw=c.stringWidth(title,'SansB',8); c.line(M+tw+6,y+3,M+CW,y+3)
    y-=lbl_h
    for i,(img,cap,is_before) in enumerate([(b_img,b_cap,True),(a_img,a_cap,False)]):
        px=M+i*(pw+0.15*inch); iy=y-ph
        rr(c,px-0.02*inch,iy-cap_h-0.03*inch,pw+0.04*inch,ph+cap_h+0.06*inch,5,fill=IVORY,stroke=LIGHT,sw=0.5)
        if img:
            try: c.drawImage(crop_img(img,ratio),px,iy,width=pw,height=ph,preserveAspectRatio=False)
            except: rr(c,px,iy,pw,ph,0,fill=LIGHT)
        else:
        ...
        badge=colors.HexColor('#DC2626') if is_before else GREEN
        pill(c,px+0.07*inch,iy+ph-0.22*inch,0.52*inch,0.165*inch,
             'BEFORE' if is_before else 'AFTER',badge,WHITE,size=6.3)
        c.setFillColor(NAVY); c.rect(px,iy-cap_h,pw,cap_h,fill=1,stroke=0)
        c.setFont('SansB',7.5); c.setFillColor(WHITE)
        c.drawCentredString(px+pw/2,iy-cap_h+0.067*inch,cap[:35])
    return y-ph-cap_h-0.14*inch

def single_photo(c, y, img, cap, ph):
    pw=(CW-0.15*inch)/2; ratio=pw/ph; cap_h=0.20*inch
    px=M; iy=y-ph
    rr(c,px-0.02*inch,iy-cap_h-0.03*inch,pw+0.04*inch,ph+cap_h+0.06*inch,5,fill=IVORY,stroke=LIGHT,sw=0.5)
    if img:
        try: c.drawImage(crop_img(img,ratio),px,iy,width=pw,height=ph,preserveAspectRatio=False)
...
def notes_box(c, y, text):
    if not text: text='No notes entered.'
    words=text.split(); lines=[]; line=''
    c.setFont('Sans',8.5)
    for w in words:
        t=(line+' '+w).strip()
        if c.stringWidth(t,'Sans',8.5)<CW-0.3*inch: line=t
    ...
    for l in lines: c.drawString(M+0.13*inch,ty,l); ty-=0.165*inch
    return y-bh-0.13*inch

# ── Rec cards ─────────────────────────────────────────────────────────────
def rec_cards(c, y, job):
    cw=(CW-2*0.12*inch)/3; ch=0.62*inch
    recs=[
...
def sig_line(c, y, job):
    ...
    c.setFont('Sans',7.5); c.setFillColor(MID)
    date=format_date(job.get('date','-')); tech=job.get('tech','-')
...
def build_pdf(job):
    ...
    c.setTitle(f'Pro-Fresh Houston - {job.get("clientName","Client")}')
    ...
    # PAGE 1
    draw_header(c,job,1,2); draw_footer(c)
    y=H-1.44*inch
    y=sec_hdr(c,y,'Job Information'); y-=0.07*inch
    y=info_cards(c,y,job); y-=0.12*inch
    y=sec_hdr(c,y,'Services Performed'); y-=0.07*inch
    if services: y=services_table(c,y,services)
    y-=0.10*inch
    y=sec_hdr(c,y,'Photo Documentation'); y-=0.09*inch

    valid=[p for p in pil_photos if p is not None]
    if valid:
        n_pairs=(len(valid)+1)//2
        ...
        while i<len(valid):
            if i+1<len(valid):
                b=valid[i]; a=valid[i+1]
                title=b['label'].lower().replace('before','').replace('after','').strip().title() or 'Photo'
                y=photo_pair(c,y,title,b['img'],a['img'],b['label'],a['label'],ph)
                i+=2
            else:
                sole=valid[i]
    ...
    # PAGE 2
    c.showPage()
    draw_header(c,job,2,2); draw_footer(c)
    y=H-1.44*inch
    y=sec_hdr(c,y,'Technician Notes'); y-=0.08*inch
    y=notes_box(c,y,job.get('notes','')); y-=0.22*inch
    y=sec_hdr(c,y,'Recommendations'); y-=0.10*inch
    y=rec_cards(c,y,job); y-=0.22*inch
    y=sec_hdr(c,y,'Technician Sign-Off'); y-=0.14*inch
    sig_line(c,y,job)
...
def health():
    try:
        ...
        pv=Image.__version__
    except Exception as e:
        pv=str(e)
    return jsonify({'status':'ok','service':'Pro-Fresh PDF Generator','pillow':pv})
...
def generate_report():
    ...
    try:
        ...
        name=job.get('clientName','Client').replace(' ','_')
        return send_file(pdf_buf,mimetype='application/pdf',
                        as_attachment=True,
                        download_name=f'ProFresh_Report_{name}.pdf')
    except Exception as e:
        import traceback
